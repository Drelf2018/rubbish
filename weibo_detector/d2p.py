...
def create_new_img(userInfo: list, post: list, headers=None, w=1080) -> Image:
    """
    根据微博博文生成图片
    Args:
        userInfo (list) : 博主用户信息
        post (list) : 博文信息
        w (int) : 生成图片的宽度
    ...
    # 定义字体
    font_type = 'C:/Windows/Fonts/msyh.ttc'
    ...
    pics = []
    if 'PicAll' in post and post['PicAll']:
        # 多张图片
        for pic in post['PicAll']:
            ...
            pics.append(bg)
    elif 'PicOri' in post and post['PicOri']:
        # 仅一张
        response = requests.get(post['PicOri'], headers=headers)  # 请求图片
        bg = Image.open(BytesIO(response.content))  # 读取图片
        bg = bg.resize((int(text_width*w), int(bg.height*text_width*w/bg.width)), Image.ANTIALIAS)  # 调整大小
        picHeight += bg.height + 3
        pics.append(bg)
    ...
    # 背景发卡
    faka = Image.open(os.path.join(sys.path[0], '发卡.png'))
    ...
    if post['repo']:
        draw.rectangle([
            (int((1-text_width)/2*w-10), int(0.315*w+now_height[split_num][0])),
    ...
    # 二维码
    url = 'https://m.weibo.cn/status/'+post['Mid']
    ...
    draw.text((int(0.21*w), int(0.14375*w)), '粉丝 {foer} | 关注 {foll}'.format_map(userInfo), '#bebebe', follow_font)
